 ...
 *
 * @package    qformat_blackboard_socot
...
require_once($CFG->dirroot . '/question/format/blackboard_socot/formatbase.php');
require_once($CFG->dirroot . '/question/format/blackboard_socot/formatqti.php');
require_once($CFG->dirroot . '/question/format/blackboard_socot/formatpool.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qformat_blackboard_socot_file {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qformat_blackboard_socot extends qformat_blackboard_socot_base {
    /** @var int Blackboard assessment qti files were always imported by the blackboard_socot plugin. */
    ...
    public function readdata($filename) {
        ...
        if (strtolower(pathinfo($filename, PATHINFO_EXTENSION)) == 'dat') {
            ...
            $fileobj = new qformat_blackboard_socot_file();
     ...
     * so fingers crossed!
     *
     * @param array $lines array of qformat_blackboard_socot_file objects for each input file.
    ...
    public function readquestions($lines) {
        ...
        // Each element of $lines is a qformat_blackboard_socot_file object.
        foreach ($lines as $fileobj) {
            if ($fileobj->filetype == self::FILETYPE_QTI) {
                $importer = new qformat_blackboard_socot_qti();
            } else if ($fileobj->filetype == self::FILETYPE_POOL) {
                $importer = new qformat_blackboard_socot_pool();
            } else {
        ...
        foreach ($questions as $question) {
            if ($question->qtype == 'category' && $question->category == '') {
                $question->category = get_string('importedcategory', 'qformat_blackboard_socot', ++$unnamedcount);
            }
        }
