 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qformat_blackboard_socot_qti extends qformat_blackboard_socot_base {
    /**
        ...
        foreach ($presentation->blocks as $pblock) {
            ...
            switch($block->type) {
                ...
                case 'RIGHT_MATCH_BLOCK':
                    ...
                    break;
                default:
                    $this->error(get_string('unhandledpresblock', 'qformat_blackboard_socot'));
                    break;
            }
...
/*                $block->filename = $this->getpath($curblock,
    ...
    public function process_common($quest) {
        ...
        $question->name = $this->create_default_question_name($question->questiontext,
                get_string('defaultname', 'qformat_blackboard_socot' , $quest->id));
    ...
    public function process_matching($quest, &$questions) {
        ...
        if ($subquestioncount < 2 || $subanswercount < 3) {
                $this->error(get_string('notenoughtsubans', 'qformat_blackboard_socot', $question->questiontext));
        } else {
            $questions[] = $question;
        }
    }
